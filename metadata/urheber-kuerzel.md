# Urheber-Kürzel

Referenzliste für die Bild-Namenskonvention `<Urheber-Kürzel>_<Kurzbeschreibung>[_<Jahr>].<ext>`.

Regel: Personen erhalten zwei Buchstaben, Institutionen/Firmen/unbekannte Urheber durchgängig drei Buchstaben.

## Personen

| Kürzel | Person |
|---|---|
| OG | Otto |
| AG | Astrid |
| MG | Martin |
| SM | Stephan Michelon |
| RH | Regina |
| HV | Helmut Vogt |
| CP | Christian Pausinger (auf der org. Website nur als "Christian" bezeichnet) |
| SO | Susi Guperding |
| DG | Dominik |
| KG | Klaus Gambos |
| AB | Adelheid |
| HB | Baumgartner Hubert |
| FS | Frieda |
| MH | Schreier Marie |
| CS | Karl Heinz |
| MG2 | Magdi (Source-Ordner `Wasser/Renovierung/magdi *.JPG`) — Kürzel-Kollision mit MG (Martin), daher Zahlensuffix statt eines zweiten Zwei-Buchstaben-Kürzels |

## Institutionen / Firmen

| Kürzel | Quelle |
|---|---|
| GTC | Gemeinde Tuntenhausen Chronik (vormals getrennt als "Gemeindechronik" und "Chronik Buch" geführt — laut Dateizeitstempeln dieselbe Digitalisierungs-Sitzung, also dieselbe physische Quelle) |
| WBV | Dias WBV (Wasserversorgungsverband) |
| PRN | Perner |
| SHC | Sindlhausen Chronik — Bilder ohne eigenen Personen-Unterordner im Source-Baum (direkt im jeweiligen Themenordner abgelegt), die aus dem Chronik-Bestand statt von einer einzelnen Person stammen |

## Sonderfall

| Kürzel | Bedeutung |
|---|---|
| UNK | Unbekannt (Unknown) — Urheber nicht ermittelbar |
